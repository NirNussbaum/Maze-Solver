...
public class Maze { 
  ...
  /** 
   ...
   public void connect (int x1, int y1, int x2, int y2) {  
      if(areConnected(x1, y1, x2, y2)) return;
		if(image.isOn(x1, y1) ==  image.isOn(x2, y2)) {
         int uniquepix1 = pixelToId(x1, y1);
         int uniquepix2 = pixelToId(x2, y2);
         uf.union(uf.find(uniquepix1),uf.find(uniquepix2));
      }
   } 
   ...
   public boolean areConnected (int x1, int y1, int x2, int y2) { 
		int uniquepix1 = pixelToId(x1, y1);
      int uniquepix2 = pixelToId(x2, y2);
      if(uf.find(uniquepix1) == uf.find(uniquepix2)) {
         return true;
      }
		return false;
   } 
   ...
   public static void main (String[] args) { 
	   Maze maze = new Maze ("maze_images/maze1.PNG", Color.white); 
